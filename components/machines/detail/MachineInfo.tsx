import { Machine, POF, MachineStatus, MachineType } from "@prisma/client";
...
export function MachineInfo({ machine, onEdit }: Props) {
  return (
    <div className="grid md:grid-cols-2 gap-6 bg-background rounded-lg p-4 border">
      ...
      <div>
        <p className="text-sm text-muted-foreground">POF</p>
        <p>{machine.pof?.name ?? "–"}</p>
      </div>
