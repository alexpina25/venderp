          ...
          <Card key={route.id}>
            <CardHeader className="pb-2">
              <CardTitle className="text-lg">
                Ruta {idx + 1} - {format(new Date(route.date), "PPP", { locale: es })}
              </CardTitle>
              <Badge variant={variant} className="text-xs w-fit">{label}</Badge>
            </CardHeader>
            <CardContent className="space-y-1 text-sm text-muted-foreground">
              <p>
                <strong>Operario:</strong> {route.operator?.name ?? "Sin asignar"}
              </p>
              <p>
